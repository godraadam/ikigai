>>>> -
>>>> Software Engineering final project, documentation. <br>
>>>> Team: Godra Adam, Benedek Balazs <br>
>>>> -
>>>> Title: IKIGAI (生き甲斐)
>>>> - 
>>>> -
...
The important part from the html file: 
<img src= "./carbon.png">

An important part from the javascript file: 
...
The coloring part for the map, when they are colorized by their rate, to give this chance to the map we need a lot of users, until that the alternative coloring method is by "visited or not".

<img src="./color.png">

The countries are saved in a HAshMap, to be honest, they are hardcoded:

<img src = "./co.png">

# `Accesability`

The api calls do the following
-  `127.0.0.1:8080/api/user/name/{username}` -> this get request returns the user and the visited countries, from where we can easily set up the map, where the user was, or where wasn't.
-  `127.0.0.1:8080/api/user/visit/{username}` -> this post request together with a json text puts in the database the user's visitted countries, whose attributes are taken from the map, while the user clicks on it. This request is used at the login page as well, where we need the password verification, if the user wrote the exact password he has.
-  `127.0.0.1:8080/api/user/register` -> together with a json text it inserts the user into the database, which signed up currently.
-  `127.0.0.1:8080/api/user/login` -- get request for login too
-  `127.0.0.1:8080/api/user/{id}` -- put request for changing the password
-  `127.0.0.1:8080/api/visit/{name}` --get request for all visits of a user
...
As soon as the user opens the page, he can find the login page. If he does not have an account he can easily sign up by hitting the sign up button. Basically, the user can switch pretty nice between the login and the register pages.
The username (email) and password are required in each case.

<img src = "./login.png">
<img src = "./register.png">

When the user enters succesfully, he can see the map, with his own visitted countries, which can be updtaed as well.
...
After we finished this project, I reckon that our mind has widened while we were able to dive into the Java + Spring implementation, improving our java skills, learning new techniques, and working more beautiful than until now. There is no doubt this project was a good beginning for our set of experiences. 

This assignment helped me to be more familiar with the Java and Spring, but I still need exercise to understand better its power and to be able to use it in every situation that becomes simpler to solve with these methods.
...
# `Bibliography`
[REST Controllers in Java using Spring](https://www.baeldung.com/category/rest/) <br>
[Spring Data JPA](https://www.baeldung.com/the-persistence-layer-with-spring-data-jpa)<br>
[Dependency Injection in Spring](https://www.baeldung.com/inversion-control-and-dependency-injection-in-spring)<br>
[MDP based recommendation system](https://www.jmlr.org/papers/volume6/shani05a/shani05a.pdf)<br>
[World map charts with svgMap - by Stephan Wagner](https://stephanwagner.me/create-world-map-charts-with-svgmap)<br>